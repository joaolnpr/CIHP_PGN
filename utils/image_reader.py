...
def parse_image(img_path, label_path, edge_path, input_size, random_scale, random_mirror):
...
def create_dataset(data_dir, data_list, data_id_list, input_size, random_scale, random_mirror, batch_size, shuffle=True):
    image_list, label_list = read_labeled_image_list(data_dir, data_list)
    edge_list = read_edge_list(data_dir, data_id_list)
    dataset = tf.data.Dataset.from_tensor_slices((image_list, label_list, edge_list))
    if shuffle:
        dataset = dataset.shuffle(buffer_size=len(image_list))
    dataset = dataset.map(lambda img, lbl, edge: parse_image(img, lbl, edge, input_size, random_scale, random_mirror),
